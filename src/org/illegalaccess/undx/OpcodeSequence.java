...
public class OpcodeSequence extends ArrayList<DalvikCodeLine> {
	...
	public DalvikCodeLine getByLogicalOffset(String z ) {
		OpcodeSequence i = this;
		for (Iterator<DalvikCodeLine> dcl = i.iterator(); dcl.hasNext();) {
			DalvikCodeLine theLine = dcl.next();
			//System.out.println(z+":"+theLine);
			if (theLine.getPos()==Integer.parseInt(z,16)) {
				return theLine;
			}
		}
		return null;
	...
	public boolean add(DalvikCodeLine last) {
		if (size() > 0) {
			DalvikCodeLine prev = this.get(size() - 1);
			prev.setNext(last);
			last.setPrev(prev);
		}
